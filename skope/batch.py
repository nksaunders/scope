...
def Simulate(arg):
    ...
    if os.path.isfile('batch/plot_run4/%2dmag%.2fmotion%.2f.npz' % (iter, mag, m_mag)):
    ...
    # create missing lc
        ...
        np.savez('batch/plot_run4/%2dmag%.2fmotion%.2f' % (iter, mag, m_mag), fpix=fpix, flux=flux)
...
def Benchmark():
    ...
    for iter in range(niter):
        ...
        for i, mag in enumerate(mags):
            flux = np.load('batch/plot_run4/%2dmag%.2fmotion%.2f.npz' % (iter, mag, 0.))['flux']
            cdpp[i] = CDPP(flux)
        if iter == 0:
    ...
    for iter in range(niter):
        cdpp = np.zeros_like(mags)
        for i, mag in enumerate(mags):
            flux = np.load('batch/plot_run4/%2dmag%.2fmotion%.2f.npz' % (iter, mag, 1.))['flux']
            cdpp[i] = CDPP(flux)
        if iter == 0:
    ...
    for m_mag, color in zip([1, 2, 5, 10, 20], ['b', 'g', 'y', 'orange', 'k']):
        ...
        for i, mag in enumerate(mags):
            for iter in range(niter):
                flux = np.load('batch/plot_run4/%2dmag%.2fmotion%.2f.npz' % (iter, mag, m_mag))['flux']
