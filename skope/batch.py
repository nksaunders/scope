...
def Simulate(arg):
    ...
    sK2 = skope.Target(ftpf = os.path.expanduser('~/.kplr/data/k2/target_pixel_files/205998445/ktwo205998445-c03_lpd-targ.fits.gz'))
...
def Benchmark():
    ...
    for iter in range(niter):
        ...
        for i, mag in enumerate(mags):
            flux = np.load('batch/%2dmag%.2fmotion%.2f.npz' % (iter, mag, 0.))['flux']
            cdpp[i] = CDPP(flux)
        if iter == 0:
    ...
    for iter in range(niter):
        cdpp = np.zeros_like(mags)
        for i, mag in enumerate(mags):
            flux = np.load('batch/%2dmag%.2fmotion%.2f.npz' % (iter, mag, 1.))['flux']
            cdpp[i] = CDPP(flux)
        if iter == 0:
    ...
    for m_mag, color in zip([1, 2, 5, 10, 20], ['b', 'g', 'y', 'orange', 'k']):
        ...
        for i, mag in enumerate(mags):
            for iter in range(niter):
                flux = np.load('batch/%2dmag%.2fmotion%.2f.npz' % (iter, mag, m_mag))['flux']
